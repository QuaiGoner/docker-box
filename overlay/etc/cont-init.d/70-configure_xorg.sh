...
function configure_x_server {
    ...
    if ([ "X${monitor_connected}" = "X" ] ); then
        ...
        cp -f /templates/xorg/xorg.dummynv.conf /etc/X11/xorg.conf
    fi
	if ( ([ "${FORCE_X11_DUMMY_CONFIG}" = "true" ] && [ -z "${nvidia_gpu_hex_id}" ] )) ; then
		...
		cp -f /home/default/edid.bin /etc/X11/edid/edid.bin
    fi
}
